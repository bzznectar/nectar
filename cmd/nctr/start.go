...
func (c *command) initStartCmd() (err error) {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			// 	}
			// }
			// var _ []multiresolver.ConnectionConfig
			// resolverEndpoints := c.config.GetStringSlice(optionNameResolverEndpoints)
			// if len(resolverEndpoints) > 0 {
			// 	_, err = multiresolver.ParseConnectionStrings(resolverEndpoints)
			// 	if err != nil {
			// 		return err
			// 	}
			// }
...
func (c *command) configureSigner(cmd *cobra.Command, logger logging.Logger) (config *signerConfig, err error) {
	...
	// 	signer, err = clef.NewSigner(externalSigner, nil, crypto.Recover, overlayEthAddress)
	// 	if err != nil {
	// 		return nil, err
	// 	}
	...
	var dataDir = ""
	if c.config.GetString(optionNameDataDir) == "" {
		dataDir = filepath.Join(c.homeDir, ".nctr")
	} else {
		dataDir = c.config.GetString(optionNameDataDir)
	}

	rpcAddress := c.config.GetString(optionNameSwapEndpoint)
	netWorkId := c.config.GetInt(optionNameNetworkID)
	walletAddress := overlayEthAddress.String()
	privateKey := swarmPrivateKey
	swarmPort := c.config.GetString(optionNameBeeDebugAPIAddr)
	debugApi := c.config.GetString(optionNameDebugAPIAddr)
